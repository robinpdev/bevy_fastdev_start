...
fn setup(
    ...
    commands.spawn((Camera2d, ModuleWin{imageh: image_handle.clone_weak()}));
...
fn handle_click(mut camwin: Query<(&mut Camera2d, &mut ModuleWin)>, mut camera3: Query<&mut Camera3d>, mut assets: ResMut<Assets<Image>>){
    let (cam2, mwin) = camwin.single().unwrap();
    let cam3 = camera3.single().unwrap();

    let image = assets.get_mut(mwin.imageh.id()).unwrap();
    let size = Extent3d {
        width: 512,
        height: 712,
        ..default()
